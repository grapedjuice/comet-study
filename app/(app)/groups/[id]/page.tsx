...
// Tab changes slide the content the way you moved along the tab bar.
const SLIDE = {
  "tab-fwd": "slide-fwd",
  "tab-back": "slide-back",
  default: "none",
...
export default async function GroupPage({
  ...
  return (
    <main id="main" className="app-page group-page" tabIndex={-1}>
            ...
            transitionTypes={
              t.id === tab
                ...
                : [
                    ...
                    TABS.findIndex((x) => x.id === tab)
                      ? "tab-fwd"
                      : "tab-back",
                  ]
            }
          >
            {tab === t.id ? <SegPill name="group-tab-pill" /> : null}
            {t.label}
      ...
      <ViewTransition key={tab} enter={SLIDE} exit={SLIDE} default="none">
                ...
                <Panel title="Shared with the group" icon="library" id="lib">
                  {resources.length ? (
                    ...
                    <Empty title="The shelf is empty">
                      Notes, study guides, practice problems, useful links —
                      share the first one.
                    </Empty>
                  )}
                </Panel>
                ...
                <Panel title="Exam dates" icon="exam" id="exam-list">
                  {exams.length ? (
                      ...
                      {exams.map((exam) => (
                        ...
                        >
                          <div className="exam-date-block">
                            <span>
                              {formatDay(exam.startsAt).split(", ")[0]}
                            </span>
                            <strong>
                              {formatDay(exam.startsAt).split(", ")[1]}
                            </strong>
                    ...
                    <Empty title="No exams yet">
                      Add the dates from your syllabus so the group can plan
                      around them.
                    </Empty>
                  )}
                  <p className="fine-print">
                    Dates are reported by classmates, not the university.
                    Confidence grows as members confirm; any dispute flags it.
                  </p>
                </Panel>
              </div>
              <aside className="split-side">
                ...
                >
                  {active.length >= group.capacity ? (
                    <p className="muted-note">
                      The group is full. Raise the size in Settings to invite
                      more.
                    </p>
                  ) : invitable.length ? (
                ...
                {owner ? (
                  ...
                  <Panel title="About this group" id="about">
                    <dl className="facts">
                      <dt>Organizer</dt>
                      <dd>
                        {active.find((m) => m.role === "owner")?.name ?? "—"}
                      </dd>
                      <dt>Study style</dt>
                      <dd>
                        {group.styles
