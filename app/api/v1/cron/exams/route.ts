...
/** Daily exam-date refresh (registrar windows + room schedule), with CRON_SECRET. */
export async function GET(request: Request) {
  ...
  if (!nebula)
    return apiError("EXAMS_UNAVAILABLE", "Live data is disabled", 503);
  try {
    return ok(await syncCampusExams(getDatabase(env.DATABASE_URL), nebula));
  } catch {
    return apiError("EXAMS_UNAVAILABLE", "Exam sync failed", 503);
  }
}
